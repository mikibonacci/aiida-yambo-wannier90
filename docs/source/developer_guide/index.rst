...
``aiida-yambo-wannier90`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-yambo-wannier90`` repository (preferably using ``aiida-yambo-wannier90`` as the project name)

The documentation is now available at `aiida-yambo-wannier90.readthedocs.io <http://aiida-yambo-wannier90.readthedocs.io/>`_.
