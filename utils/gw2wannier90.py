...
IKP = [
    tuple(
        np.array(np.round(np.array(f.readline().split(), dtype=float) * n1), dtype=int)
    )
    for i in range(NKPT)
]
...
KPNB = np.array(
    [[int(f.readline().split()[1]) - 1 for inb in range(NNB)] for ikpt in range(NKPT)]
)
        ...
        for ik in range(NKPT):
            for ib2 in range(nnb):
                for ib1 in range(nnb):
                    ...
                    A = (
                        A.reshape(nbnd, nbnd, order="F")[BANDSORT[KPNB[ik][ib2]], :][
...
if calcSPN:
    try:
        ...
        for ik in range(NK):
            ...
            A = A[:, :, BANDSORT[ik]][:, BANDSORT[ik], :][:, indnQP, indmQP].reshape(
                (3 * NBND * (NBND + 1) // 2), order="F"
            )
